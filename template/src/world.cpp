...
bool World::init(vec2 screen)
{
    ...
    m_break_barrel_sound = Mix_LoadWAV(audio_path("barrel_break.wav"));
    m_enemy_hit_sound = Mix_LoadWAV(audio_path("enemy_hit.wav"));
    m_level_up_sound = Mix_LoadWAV(audio_path("level_up.wav"));
    m_enter_level_sound = Mix_LoadWAV(audio_path("enter_level.wav"));
    ...
    if (m_background_music == nullptr || m_portal_through_sound == nullptr || m_prince_dead_sound == nullptr ||
        ...
        m_break_barrel_sound == nullptr || m_enemy_hit_sound == nullptr || m_level_up_sound == nullptr || m_enter_level_sound == nullptr)
    {
...
void World::destroy()
{
    ...
    if (m_break_barrel_sound != nullptr)
        Mix_FreeChunk(m_break_barrel_sound);
    if(m_enemy_hit_sound != nullptr)
        Mix_FreeChunk(m_enemy_hit_sound);
    if(m_level_up_sound != nullptr)
        Mix_FreeChunk(m_level_up_sound);
    if(m_enter_level_sound != nullptr)
        Mix_FreeChunk(m_enter_level_sound);
    ...
    for (auto &heart : m_hearts_bar)
        heart.destroy();
    m_hearts_bar.clear();
    for (auto &heart : m_grey_hearts_bar)
        heart.destroy();
    m_grey_hearts_bar.clear();
...
bool World::update(float elapsed_ms)
{
    ...
    else if (game_state == game_play)
    {
        ...
        {   
            Mix_PlayChannel(-1, m_level_up_sound, 0);
        ...
        if (len(m_prince.m_velocity) > 0.f)
        {
            if (m_prince.m_velocity.x < 0.f)                                                  // left
                ...
                m_pebbles_emitter.spawn_pebble(add(m_prince.get_position(), {0.f, 0.f}), 90); // smoke on up
        }
        ...
        star_bar.update(0);
        arrow.update(elapsed_ms);
        for (auto &heart : m_hearts_bar)
            heart.update(elapsed_ms);
        // printf("number of hearts in heart bar: %d\n", m_hearts_bar.size());
        for (auto &heart : m_level.camera_views[curr_view].m_hearts)
            heart.update(elapsed_ms);
        for (auto &wall : m_walls)
            wall.update(elapsed_ms);
        for (auto &barrel : m_level.camera_views[curr_view].m_barrels)
            barrel.update(elapsed_ms);
        ...
        {
            enemy.update(elapsed_ms, m_prince.get_position(), m_walls, m_level.camera_views[curr_view].m_barrels);
            Bullet *bullet = &m_bullets_enemy[enemy.get_key()];
            if (enemy.get_stop_and_attack() && bullet->get_state() == 0 && bullet->is_ready())
            {
                bullet->set_state(1);
                bullet->set_position(enemy.get_position());
                bullet->set_rotation(enemy.get_angle());
                bullet->update(elapsed_ms); // need this to avoid bullet - enemy collision
            }
            bullet->update(elapsed_ms);
        }

        for (auto &prince_bullet : m_bullets_prince)
            prince_bullet.update(elapsed_ms);
        m_popup_window.update(elapsed_ms);
        return true;
    }
...
void World::check_collisions(float ms)
{
    ...
    for (auto &enemy : m_level.camera_views[curr_view].m_enemies)
    {
        ...
        Bullet *bullet = &m_bullets_enemy[enemy.get_key()];
        auto bullet_p = m_bullets_prince.begin();
        while (bullet_p != m_bullets_prince.end())
        {
            if (CollisionManager::bullet_collides_with_bullet(bullet, *bullet_p))
            {
                bullet->set_position(enemy.get_position());
                bullet->set_state(2);
                bullet_p = m_bullets_prince.erase(bullet_p);
            }
            else
                ++bullet_p;
        }
        ...
        else if (CollisionManager::bullet_collides_with_boundry(bullet))
        {
            bullet->set_position(enemy.get_position());
            bullet->set_state(2);
        }
        else
        {
            for (auto &wall : m_walls)
            {
                if (CollisionManager::bullet_collides_with_wall(bullet, wall))
                {
                    bullet->set_position(enemy.get_position());
                    bullet->set_state(2);
                    break;
            ...
            for (auto &barrel : m_level.camera_views[curr_view].m_barrels)
            {
                if (CollisionManager::bullet_collides_with_barrel(bullet, barrel))
                {
                    bullet->set_position(enemy.get_position());
                    bullet->set_state(2);
                    break;
        ...
        CollisionManager::prince_collides_with_key(&m_prince, m_level.camera_views[curr_view].m_opt_key.m_key))
    {
        ...
        if (current_level == 0)
        {
            ...
            game_state = game_end;
            m_background.set_end_screen_img_path();
            m_background.change_background();
        }
        else
        {
    ...
    // Checking Prince and enemy - wall collisions
    ...
    // Checking Prince and enemy - barrel collisions
    auto barrel_it = m_level.camera_views[curr_view].m_barrels.begin();
    while (barrel_it != m_level.camera_views[curr_view].m_barrels.end())
    {
        if (m_prince.is_alive() && CollisionManager::prince_collides_with_barrel(&m_prince, *barrel_it, ms))
        {
            float vector_x = m_prince.get_position().x - barrel_it->get_position().x;
            float vector_y = m_prince.get_position().y - barrel_it->get_position().y;
            vec2 normalize_vector = normalize({vector_x, vector_y});
            m_prince.move(mul(normalize_vector, 6));
            m_prince.m_velocity = {0.f, 0.f};
            break;
        ...
        else
        {
            for (auto &enemy : m_level.camera_views[curr_view].m_enemies)
            {
                if (enemy.collides_with(*barrel_it, ms))
                {
                    vec2 vector = sub(enemy.get_guard_position(), enemy.get_position());
                    if (len(vector) < 0.001f)
                        break;
    ...
    auto bullet_it = m_bullets_prince.begin();
    while (bullet_it != m_bullets_prince.end())
    {
        ...
        while (enemy != m_level.camera_views[curr_view].m_enemies.end())
        {
            ...
            {   
                ...
                bullet_it->destroy();
                bullet_it = m_bullets_prince.erase(bullet_it);
                enemy->minus_life();
                Mix_PlayChannel(-1, m_enemy_hit_sound, 0);
        ...
        for (auto &barrel : m_level.camera_views[curr_view].m_barrels)
        {
            if (CollisionManager::bullet_collides_with_barrel(&*bullet_it, barrel))
            {
                bullet_it->destroy();
                bullet_it = m_bullets_prince.erase(bullet_it);
                //break barrel
                Mix_PlayChannel(-1, m_break_barrel_sound, 0);
                continue_running = true;
                if (!barrel.get_is_broken())
                {
                    if (current_level == 0)
                    {
                        barrel.hit_by_bullet();
                        int num_stars = 3;
                        m_prince.m_points += num_stars;
                        m_popup_window.show_window(num_stars);
                    }
                    else
                    {
                        int num_stars = barrel.hit_by_bullet();
                        m_prince.m_points += num_stars;
                        m_popup_window.show_window(num_stars);
                        if (m_prince.m_points > 99)
                            m_prince.m_points = 99;
                    }
                }
                break;
            }
...
void World::update_screen(int pos)
{
    ...
    for (auto &barrel : m_level.camera_views[get_curr_view()].m_barrels)
        barrel.clear_pebbles();
    Barrel::set_texture_change();
    m_popup_window.set_disappear();
    assign_background();
    m_pebbles_emitter.destroy();
    m_pebbles_emitter.init();
    //    for (auto &bullet : m_bullets_prince)
    //        bullet.destroy();
    //    m_bullets_prince.clear();
    //    for (int i = 0; i < m_bullets_enemy.size(); i++)
...
void World::draw()
{
    ...
    title_ss << "The Little Prince and the Lost Rose";
    ...
    else if (game_state == game_play)
    {
            ...
            if (current_level == 0 && get_curr_view() != m_level.camera_views.size() - 1)
            ...
            {
                Bullet *bullet = &m_bullets_enemy[enemy.get_key()];
                if (bullet->get_state() == 1)
                    bullet->draw(projection_2D);
                enemy.draw(projection_2D);
            }
            for (auto &barrel : m_level.camera_views[get_curr_view()].m_barrels)
                barrel.draw(projection_2D);
            ...
            for (int i = m_prince.get_lives(); i < 5; i++)
                m_grey_hearts_bar[i].draw(projection_2D);
            ...
            m_popup_window.draw(projection_2D);
    ...
    else if (game_state == game_narration)
    {    
        ...
        if(capture.isOpened())
        {
            ...
            if(!capture.read(frame) || s_key_press)
            {   
                 // destroy frame texture
                 m_frame.destroy();
                 //cv::destroyWindow("Narration");
                 clear_camera_view();
                 Walls::set_texture_change();
                 Enemy::set_texture_change();
                 current_level = 0;
            ...
            {   
                //cv::imshow("Narration", frame);
                // mat to texture
                matToTexture(frame);
                //printf("%u\n", m_frame.frame_texture.id);
                m_frame.draw(projection_2D);
...
GLuint World::matToTexture(const cv::Mat &mat) {
    GLuint textureID;
    // glGenTextures(1, &textureID);
    textureID = m_frame.get_texture_id();
...
bool World::check_ontop_assests(vec2 pos)
{
    for (auto star : m_level.camera_views[m_level.portal_view].m_stars)
    {
        if (len(sub(pos, star.get_position())) < 100 * sqrt(2))
            return true;
    }
    for (auto heart : m_level.camera_views[m_level.portal_view].m_hearts)
    {
        if (len(sub(pos, heart.get_position())) < 100 * sqrt(2))
            return true;
    }
    return false;
}
...
void World::spawn_portal()
{
    if (m_level.camera_views[m_level.portal_view].m_opt_portal.m_portal.init())
    {
        ...
        vec2 rand_pos = {0.f, 0.f};
        float dist_pos = 0.f;
        bool is_good_position = true;

        while (is_good_position)
        {
            rand_num = rand() % matrix_positions.size();
            rand_pos = matrix_positions[rand_num];
            dist_pos = sq_len(sub(m_level.camera_views[m_level.portal_view].m_opt_key.m_key.get_position(), rand_pos));
            is_good_position = dist_pos >= 5.f * (float)(100 * 100);
            is_good_position = is_good_position || dist_pos <= (float)(100 * 100);
            is_good_position = is_good_position || check_ontop_assests(rand_pos);
        }
...
void World::play_intro_video()
{   
    capture.open(video_path("narration.mp4"));
    m_frame.init();
...
bool World::spawn_barrels(int level, int view)
{
    for (int i = 0; i < 12; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            {
                Barrel barrel;
                if (!barrel.init(level))
                {
                    fprintf(stderr, "Failed to spawn barrels");
                    return false;
                }
                m_level.camera_views[view].m_barrels.emplace_back(barrel);
                // printf("is a barrel\n");
                Barrel &new_barrel = m_level.camera_views[view].m_barrels.back();
                new_barrel.set_position(grid[i][j]);
                Barrel::unset_texture_change();
            }
        }
    }
    printf("this is one view \n");
    return true;
}
...
void World::on_key(GLFWwindow *, int key, int, int action, int mod)
{
    if (action == GLFW_PRESS)
    {
        ...
        else if (key == GLFW_KEY_C && game_state == game_start)
        {
            c_key_press = true;
            load_save_file();
            update_screen(5);
        ...
        else if (key == GLFW_KEY_S && game_state == game_play)
        {
            write_to_save_file();
        }
        else if (key == GLFW_KEY_Y && game_state == game_quit_screen)
        {
            y_key_press = true;
        }
        ...
        else if (key == GLFW_KEY_ENTER && game_state == game_level_screen)
        {   
            Mix_PlayChannel(-1, m_enter_level_sound, 0);
        ...
        else if (key == GLFW_KEY_SPACE && game_state == game_play && m_prince.m_points != 0)
        {
            if (!spawn_prince_bullet())
                return;
            Bullet &new_bullet = m_bullets_prince.back();
            ...
            {
                switch (m_prince.get_facing_direction())
                {
                case 0: // down
                    prev_prince_v = M_PI;
                    break;
                case 1: // up
                    ...
                    break;
                case 2: // left
                    prev_prince_v = -1.f * M_PI / 2;
                    break;
                case 3: // right
                    prev_prince_v = M_PI / 2;
                    break;
                default:
                    fprintf(stderr, "Direction does not exist\n\n");
                }
            }
            new_bullet.set_rotation(prev_prince_v);
            new_bullet.set_state(1);
            new_bullet.set_bullet(8);
            m_prince.m_points--;
        }
    }
...
{
    printf("\n new walls are being assigned now \n");
    for (int i = 0; i < 12; i++)
    {
        for (int j = 0; j < 8; j++)
        {
...
bool World::spawn_enemy(int max_enemies, int view_id)
{
    for (int i = 0; i < max_enemies; i++)
    {
        Enemy enemy;
        Bullet enemy_bullet;
        if (enemy.init(current_level) && enemy_bullet.init(current_level))
        {
            ...
            enemy_bullet.set_bullet(current_level);
...
bool World::spawn_prince_bullet()
{
    ...
    if (bullet.init(0))
    {
        m_bullets_prince.emplace_back(bullet);
        return true;
    }
...
bool World::set_bars()
{
    ...
    for (int j = m_grey_hearts_bar.size(); j < 5; j++)
    {
        ...
        if (new_heart.init())
        {
            new_heart.set_scale(vec2{0.08f, 0.08f});
            new_heart.set_position(vec2{20 + 45.0f * j, 20});
            m_grey_hearts_bar.emplace_back(new_heart);
        }
        else
            return false;
    }
...
void World::create_tutorial_level()
{
    ...
    m_level.key_view = 4;
    m_level.portal_view = 4;
    m_level.max_star = 1;
    m_level.max_heart = 1;
    m_level.max_enemies = 1;
    m_level.view_grid.row = 1;
    m_level.view_grid.col = 5;
    ...
    std::vector<Barrel> m_barrels02 = {};
    Barrel barrel;
    barrel.init(0);
    barrel.set_position(center_pos);
    Barrel::unset_texture_change();
    m_barrels02.emplace_back(barrel);
    ...
    Heart h03;
    h03.init();
    h03.set_position(center_pos);
    m_hearts03.emplace_back(h03);
    Camera_View cv03 = {{{}, false},
                        {{}, false},
                        m_stars03,
                        m_enemys03,
                        m_hearts03};
    ...
    std::vector<Star> m_stars04 = {};
    std::vector<Enemy> m_enemys04 = {};
    std::vector<Heart> m_hearts04 = {};
    Camera_View cv04 = {{new_key, true},
                        {new_portal, true},
                        m_stars04,
                        m_enemys04,
                        m_hearts04};
    m_level.camera_views.emplace_back(cv04);
}
...
void World::clear_camera_view()
{
    ...
    for (int i = 0; i < num_views; i++)
    {
        ...
        for (auto &wall : m_walls)
            wall.destroy();
        m_walls.clear();
        for (auto &barrel : m_level.camera_views[i].m_barrels)
            barrel.destroy();
        m_level.camera_views[i].m_barrels.clear();
