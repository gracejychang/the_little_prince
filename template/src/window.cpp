...
bool Window::init()
{
    ...
    num_stars_texture.init();
    num_stars_texture.set_scale({-0.08f, 0.12f});
    count_down = 0;
    motion.radians = M_PI;
...
void Window::draw(const mat3 &projection)
{
    if (count_down > 0)
    {
        ...
        if(num_stars > 0)
        {
            num_stars_texture.set_position({motion.position.x + 50, motion.position.y + 0});
            num_stars_texture.set_point(num_stars);
            num_stars_texture.draw(projection);
        }
            glDisable(GL_DEPTH_TEST);
    }
}
...
void Window::show_window(int number)
{
    count_down = 100;
    num_stars = number;
}
